<template>
  <div class="stocks-page">
    <PageHeader
      title="红利指数"
      :subtitle="scanDate ? `中证红利成分股 · 扫描日期：${scanDate}` : '中证红利成分股 · 轻量扫描，快速观测高股息'"
    >
      <template #actions>
...
import { getAllStocks, indexScan } from '../api'
...
async function fetchStocks(page) {
  ...
  loading.value = true
  try {
    const { data } = await getAllStocks(currentPage.value, pageSize.value, 'index')
