...
def get_top_stocks():
    ...
    with get_connection() as conn:
        ...
        if full_count > 0:
            cur.execute(
                "SELECT * FROM stock_daily_metrics WHERE date = ? AND scan_type = 'full' ORDER BY dividend_yield DESC LIMIT ?",
                (latest_date, limit),
            )
        else:
            cur.execute(
...
def get_all_stocks():
    ...
    with get_connection() as conn:
        ...
        # 总数
        cur.execute(
            f"SELECT COUNT(*) FROM stock_daily_metrics WHERE date = ? {scan_filter}",
            (latest_date,)
        )
        ...
        cur.execute(
            f"""SELECT * FROM stock_daily_metrics
               WHERE date = ? {scan_filter} ORDER BY dividend_yield DESC
               LIMIT ? OFFSET ?""",
            (latest_date, page_size, offset)
        )
        columns = [desc[0] for desc in cur.description]
